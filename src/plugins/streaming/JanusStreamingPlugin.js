...
export default class JanusStreamingPlugin extends JanusPlugin {
  ...
  create = async (
    id,
    { name, description, pin, secret, audioPort, audioPt, videoPort, videoPt }
  ) => {
    try {
      let additionalConfig = {};
      ...
      if (
        ...
        createStreamingResponse.plugindata &&
        createStreamingResponse.plugindata.data &&
        createStreamingResponse.plugindata.data.streaming === 'created'
      ) {
        return true;
      }
  ...
  destroy = async (id) => {
    try {
      ...
      if (
        destroyStreamingResponse &&
        destroyStreamingResponse.plugindata &&
        destroyStreamingResponse.plugindata.data &&
        destroyStreamingResponse.plugindata.data.destroyed === id
      ) {
        return true;
      }
